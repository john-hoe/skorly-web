 ...
 * feedback, and we never let raw links through. Tuned for id/vi/en audiences.
 */
...
// en: bet, casino, odds, parlay) + common spam brands.
const GAMBLING_RE =
  /\b(judi|togel|slot\s*(gacor|online)?|maxwin|gacor|taruhan|bandar|sbobet|parlay|mix\s*parlay|casino|kasino|cas[ií]no|bet(ting|365)?|sportsbook|sportsbet|odds|bookie|cá\s*cược|nhà\s*cái|đặt\s*cược|gambl(e|ing)|wd\s*(cepat|lancar)|deposit\s*pulsa|rtp\s*(live|slot))\b/i;
...
const PIRACY_RE =
  /\b(nonton\s*(gratis|live)?|streaming\s*(gratis|bola)?|live\s*streaming|link\s*(nonton|live|streaming)|xem\s*(trực\s*tiếp|bóng\s*đá)|full\s*match\s*free)\b/i;
