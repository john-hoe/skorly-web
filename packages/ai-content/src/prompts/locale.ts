...
export function localizePrompt(prompt: PromptResult, locale: Locale | string): PromptResult {
  ...
  const glossary = locale === "th" ? `\n\n${thaiGlossaryBlock()}` : "";
  return {
    system: `You are a senior ${lang} football journalist. Write entirely in ${lang} with native fluency and correct football terminology. Never invent statistics. Keep official team and player names unchanged.${glossary}\n\n${prompt.system}`,
