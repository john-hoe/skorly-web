-- ============================================================
-- Skorly admin overview — single RPC aggregate for dashboard metrics.
-- Apply via Supabase MCP / SQL editor / psql before relying on RPC-only reads.
--
-- Security:
-- - Function is SECURITY INVOKER and reads only with the caller's privileges.
-- - Execute is revoked from public client roles and granted only to service_role.
-- - Web calls this from server-side code with SUPABASE_SERVICE_ROLE_KEY.
-- ============================================================
...
profile_locales as (
  ...
  from (
    ...
    from unnest(array['id', 'vi', 'en', 'zh']) with ordinality as locales(label, ord)
    left join profiles p
...
comment on function public.admin_overview_stats() is
  'Server-side admin dashboard aggregate. Execute is limited to service_role.';
